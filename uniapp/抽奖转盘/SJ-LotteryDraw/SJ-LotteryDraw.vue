<template>
	<view class="lottery_container" :class="disabled?'':'disabled'">
				...
				<ul class="lottery_grid">
					<li v-for="(item, index) in grid_info_arr" :class="{ active: current_index == index && index != 8 }"
						...
						<image v-if="index != 8" class="grid_img" mode='aspectFit'
							...
							<!-- {{ index == 8 ? '抽奖' : index+1 }} -->
							<!-- 开始抽奖 -->
							<image v-if="index == 8" class="grid_img" :style="{width:'300rpx',height:'350rpx'}"
								mode='aspectFit' :src="drawStartImg" alt="开始抽奖">
					</li>
				</ul>
			</view>
			<!-- 外围边框 -->
			...
			luck_draw(event) {
				...
				if (index == 8) {
					...
					this.$emit('get_winingIndex', function(res) {
						// console.log('get_winingIndex', res);
						...
						let win = new LotteryDraw({
							...
							function(index, count) {
								// 随机跳
								const randomNum = Math.floor(Math.random() * 8) // 获取0~7之间的随机整数数
								that.current_index = randomNum;
								// 按顺序跳
								// that.current_index = index;
								// console.log('that.current_index', index, randomNum, that.current_index);
