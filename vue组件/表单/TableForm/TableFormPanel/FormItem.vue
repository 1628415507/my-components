 ...
 * @LastEditors: Hongzf
 * @LastEditTime: 2023-05-16 18:02:58
...
<template>
  <div>
    ...
    <el-input
      ...
      v-model="value[tableProp][rowIndex][item.prop]"
      :type="item.type"
      ...
      @change=" val => {item.change ? item.change(val, row, rowIndex,item) : ''} "
      @input=" val => {item.input ? item.input(val, row, rowIndex,item) : ''} "
      @blur=" val => {item.blur ? item.blur(val, row, rowIndex,item) : ''} "
    />
    <!-- 单选框radio -->
    <el-radio-group
      ...
      @change="
        radioVal => {
          item.changeRadio ? item.changeRadio(radioVal, row, rowIndex,item) : '';
        }
      "
    >
      <el-radio
    ...
    <el-checkbox-group
      ...
      @change="
        checkVal => {
          item.changeCheck ? item.changeCheck(checkVal, item, rowIndex) : '';
        }
      "
    >
      <el-checkbox
    ...
    <el-select
      ...
      @change="
        optionVal => {
          item.changeSelect ? item.changeSelect(optionVal, row, rowIndex,item) : '';
        }
      "
    >
      <el-option
    ...
    <el-cascader
      ...
      @change="
        value => {
          item.changeCascader ? item.changeCascader(value, item, rowIndex) : '';
        }
      "
    />
    {{ item.prependText }}
    ...
    <el-input-number
      ...
      @change="
        (currentValue, oldValue) => {
          item.changeNumber
            ? item.changeNumber(currentValue, oldValue, item, rowIndex)
            : '';
        }
      "
    />
    ...
    <el-time-select
      ...
      @change="
        time => {
          item.changeTime ? item.changeTime(time, item, rowIndex) : '';
        }
      "
    />
    ...
    <el-date-picker
      ...
      @change="
        date => {
          item.changeDate ? item.changeDate(date, item, rowIndex) : '';
        }
      "
    />
    ...
    <el-date-picker
      ...
      @change="
        dateTime => {
          item.changeDateTime ? item.changeDateTime(dateTime, item, rowIndex) : '';
        }
      "
    />
    <!-- 文本 -->
    <!-- <span v-else>
      <div :title="item.formatter?item.formatter(row, item,rowIndex):modelVal">
        {{ item.formatter?item.formatter(row, item,rowIndex):modelVal }}
      </div>
    </span> -->
  </div>
</template>
