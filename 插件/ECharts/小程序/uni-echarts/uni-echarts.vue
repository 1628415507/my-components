<template>
  <canvas type="2d" id="myCanvas" v-if="isUseNewCanvas" class="ec-canvas" :canvas-id="canvasId" @init="init" @touchstart="touchStart"
    @touchmove="touchMove" @touchend="touchEnd">
  </canvas>
  <canvas v-else id="myCanvas" class="ec-canvas" :canvas-id="canvasId" @init="init" @touchstart="touchStart" @touchmove="touchMove"
    @touchend="touchEnd">
  </canvas>
</template>
...
<script>
  ...
  export default {
    props: {
      ...
      formatTooltip: {//自定义Tooltip的方法
        type: Function,
      },
      formatYAxis: {//自定义y轴的标签的方法
     	  type: Function,
      }
    },
    data() {
    ...
    watch: {
      "ec.option": {
        ...
        handler(val, oldVal) {
	      ...
	      this.ec.option && this.ec.option.tooltip && (this.ec.option.tooltip.formatter = this.formatTooltip)
		    this.ec.option && this.ec.option?.yAxis?.axisLabel && (this.ec.option.yAxis.axisLabel.formatter  = this.formatYAxis)
     	  this.setOption(val);
        }
      }
    },
    ...
    methods: {
      ...
      clear(){
        if (this.$curChart?._dom) return echarts.dispose(this.$curChart._dom);
      },
      touchEnd(e) {
        if (this.ec.stopTouchEvent) {
          e.preventDefault();
