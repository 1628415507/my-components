  ...
  <div class="map-echarts">
    ...
    >
      ...
      <!-- <div class="info-text-wrap" v-if="isShowInfo">
        <span class="info-text" v-for="item in areaData" :key="item.id">
          {{ item.name }}:
          {{ item.value }}
          场
        </span>
...
export default {
  ...
  watch: {
    ...
    // 获取赛事列表数据
    areaData(newVal) {
      this.$emit('getEcData', newVal)
    },
    // 获取地址
    breadcrumbList(newVal) {
      const arr = newVal.map((item) => item.data)
      this.$emit('getMapInfo', arr)
    },
    timeIndex() {
      if (this.timeIndex === TIME_STATUS.DAY) {
        ...
        this.form.endDate = this.timePick[1]
      }
      this.reset(
