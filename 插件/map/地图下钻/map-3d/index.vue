<template>
  <div class="map-wrap">
    ...
    <div class="level-list">
      <!-- {{ info.curName }} -->
      <template v-for="(item, level) in levelList">
        <span v-if="level === levelList.length - 1" :key="item">{{ item }}</span>
        <!-- 不是最后一级 -->
        <span v-else :key="item" @click="handleBack(item)">
          {{ item }}
...
import 'echarts-gl';
import mapJson from '@/largescreen/common/geoJson/index.js'; // 中国地图数据
...
export default {
  ...
  data() {
    return {
      ...
      // 地图数据
      mapJson, // 加载地图数据
      rootName: 'china', // 最上级地图名称
      curMapName: 'china', // 当前地图名称
      curMapJson: {} // 当前地图数据
    };
  },
  computed: {
  ...
  methods: {
    ...
    // 绘制地图
    initMap(mapName = this.rootName) {
      this.curMapName = mapName;
      this.curMapJson = this.mapJson[mapName];
      // 注册地图数据
      ...
      const commonConfig = {
        type: 'map3D',
        map: mapName,
        zoom: 1.2, // this.curMapName === this.rootName ? 1.2 : 0.9, // 放大比例
      ...
      const mapOption = {
        ...
        geo3D: {
          ...
          regions: [
            {
              ...
              label: {
                ...
                show: true,
                backgroundColor: {
                  image: require('@/largescreen/common/img/map-cabled/toast-bg.svg'),
                  opacity: 1
                },
          ...
          shading: 'realistic',
          realisticMaterial: {
            detailTexture: require('@/largescreen/common/img/map-cabled/666.png'), // 材质细节的纹理贴图。!!!必须配置shading: 'realistic'时才有效
      ...
      this.$nextTick(() => {
        // 点击前解绑，防止点击事件触发多次
        echartsMap && echartsMap.off('dblclick');
        echartsMap && echartsMap.on('dblclick', this.echartsMapClick); // 绑定单击下钻
      });
    ...
    handleRegionsActive(mapOption) {
      ...
      echartsMap.on('mouseout', (params) => {
        ...
        this.regionsActive(mapOption);// 开始轮播
      });
      // 图表容器的事件（包括图表上的空白处）
    ...
    formatTooltip(params) {
      ...
      const toastBg = require('@/largescreen/common/img/map-cabled/toast-bg.svg');
      const toastIcon = require('@/largescreen/common/img/common/circle.svg');
      const str = `
...
@import "@/largescreen/common/common.scss";
