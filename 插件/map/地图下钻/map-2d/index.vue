 ...
 * @LastEditors: Hongzf
 * @LastEditTime: 2023-02-20 10:31:22
-->
<template>
  <div class="map-wrap">
    <!-- 显示层级 -->
    <div class="level-list">
      ...
      <template v-for="(item, level) in levelList">
        <span v-if="level === levelList.length - 1" :key="item">
          {{ item }}
        </span>
        <!-- 不是最后一级 -->
    ...
    <div class="map-box">
      <div id="mapId" ref="chartRef" style="width:100%;height:900px" />
    </div>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    initMap(mapName = this.rootName) {
      ...
      const comConfig = {
        ...
        zoom: this.curMapName === this.rootName ? 1.15 : 0.7, // 1,
        aspectScale: 0.9, // 长宽比
        scaleLimit: {
          min: 0.7, // 最小的缩放值
          max: 1.3 // 最大的缩放值
      ...
      const mapOption = {
        ...
        geo: [
          // 基础层，设置地图外边框样式
          {
            ...
            itemStyle: {
              areaColor: {
                image: require('@/largescreen/common/img/map-cabled/map-bg.png'), // 背景图
    ...
    handleTooltip() {
      ...
      echartsMap.on('mouseover', params => {
        // console.log('【 mouseover 】-411', params)
        // 清除其他地方的高亮
    ...
    autoShowTip() {
      ...
      this.intervalFn = setInterval(() => {
        ...
        // console.log('【 dataLength 】-392', dataLength);
